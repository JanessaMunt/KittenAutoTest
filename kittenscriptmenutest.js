...
var autoHuntEnable = false;
var autoTradeEnable = false;
var autoPraiseEnable =false;
var autoTradeLeviathansEnable = false;
var autoTradeNagasEnable = [false, .85];
...
div.innerHTML += '<button id="autoBcoinButton" style="color:red" onclick="autoSwitch(autoBcoinEnable, \'autoBcoinButton\')" style= "float:left"> Auto Trade BCoin </button>' ;//button for auto Bcoin traiding
//'	div.innerHTML += <button id="autoHunt" style="color:red" onclick="autoSwitch(3, \'autoHunt\')"> Auto Hunt </button>' ;
div.innerHTML += '<button id="autoTradeNagasButton" style="color:red" onclick="autoSwitch(autoTradeNagasEnable[0], \'autoTradeNagasButton\')"> Auto Trade Nagas </button>' ;
div.innerHTML += '<button id="autoTradeLeviathansButton" style="color:red" onclick="autoSwitch(autoTradeLeviathansEnable, \'autoTradeLeviathansButton\')"> Auto Trade Leviathans </button>' ;
//	div.innerHTML += '<button id="autoTrade" style="color:red" onclick="autoSwitch(3, \'autoTrade\')"> Auto Trade </button>' ;
div.innerHTML += '<button id="autoPraiseButton" style="color:red" onclick="autoSwitch(autoPraiseEnable, \'autoPraiseButton\')"> Auto Praise</button>' ;


for(var i=0; i<resources.length; i++){
	var resourceName = capitalizeFirstLetter(resources[i][1]);
	//toggle buttons for auto crafting each basic resource
	div.innerHTML += '<button id="newbutton" style="color:red" onclick="placeholder" style= "float:left"> placeholder </button>';  //autopopulate buttons
	...
	div.innerHTML += '<span id="newspan" title="Between 0 and 100"><input id="newinput" style="width:25px" onchange="placeholder" value="30"></span><br>';  //autopopulate inputs
...
function autoSwitch(enabled, buttonName) {
	if (enabled == false) { //check if autocraft off on for this resource
		enabled = true; //turn on autocraft
		console.log(enabled);
	...
	else if (enabled == true) { //check if autocraftis on for this resource
		enabled = false; //turn off autocraft
...
function autoCraft(){
	for(var i=0; i < resources.length; i++) { //cycle through all resources
	  ...
	  if(resources[i][4] == true && curRes> (gamePage.resPool.get(resources[i][0]).maxValue)*triggerAutoCraft){ //check if autocraft is on & if resource is at threshold to auto craft
...
function autoTradeLeviathans(){
  if(autoTradeLeviathansEnable == true && gamePage.resPool.get("unobtainium").value > 5000){//check if Leviathan trade is enabled and if enough unobtanium for trade
...
function autoTradeNagas(){
	if(autoTradeNagasEnable[0] == true && gamePage.resPool.get("minerals").value < autoTradeNagasEnable[1]*(gamePage.resPool.get("minerals").maxValue)){ //check if enabled and if under mineral threshold
...
//autohunt
function autoHunt(){
	if (autoHuntEnable== true && gamePage.resPool.get("manpower").value > (gamePage.resPool.get("manpower").maxValue - gamePage.resPool.get("manpower").perTickCashed *5)) {
